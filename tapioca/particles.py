...
def read_mandyoc_particles(path, parameters_file=PARAMETERS_FILE):
    """
    ...
    particle_ids = _get_all_particles_ids(path, particle_files)
    ...
    for step, time in zip(steps, times):
        ...
        # Read the files for the step and fill the dataset depending of the dimention
        for i in range(n_rank):
            if dimension == 2:
                x, z, particle_id, layer, cumulative_strain = np.loadtxt(
                    os.path.join(path, "step_{}-rank_new{}.txt".format(step, i)),
                    unpack=True,
                )
...
def _get_all_particles_ids(path, particle_files, first_step=0):
    """
    ...
    for i in range(len(first_step_files)):
        _, _, id_f, _, _ = np.loadtxt(
            os.path.join(path, "step_0-rank_new{}.txt".format(i)), unpack=True
        )
